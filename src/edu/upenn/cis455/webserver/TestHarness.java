...
public class TestHarness {	
	...
	private static ServletContextImpl createContext(Handler h) {
		ServletContextImpl fc = new ServletContextImpl(null);
	...
	public static void mainfunc(String[] args) throws Exception {
		...
		ServletContextImpl context = createContext(h);
		
		HashMap<String,HttpServlet> servlets = createServlets(h, context);
		
		HttpSessionImpl fs = null;
		
		for (int i = 1; i < args.length - 1; i += 2) {
			HttpServletRequestImpl request = new HttpServletRequestImpl(fs, null, null, null, null);
			...
			if (servlet == null) {
				System.err.println("error: cannot fin"
						+ "d mapping for servlet " + strings[0]);
				System.exit(-1);
			}
			...
			fs = (HttpSessionImpl) request.getSession(false);
